...
class UberEatsMarginScreen extends StatefulWidget {
  const UberEatsMarginScreen({Key? key}) : super(key: key);

  @override
  State<UberEatsMarginScreen> createState() => _UberEatsMarginScreenState();
}

class _UberEatsMarginScreenState extends State<UberEatsMarginScreen> {
  ...
  Future<void> _load() async {
    ...
    // ── Step 1: Load saved margins from Firebase — must succeed ──
    Map<String, dynamic> saved;
    try {
      saved = await _firebaseService.getUberEatsMargins();
    } catch (e) {
    ...
    // Show the form immediately with saved defaults — user can edit even if tags fail
    ...
    // ── Step 2: Load Shopify tags — optional, failure just shows empty list ──
    try {
  ...
  Future<void> _save() async {
    ...
    // Build tag margins — only save non-empty entries
    ...
    try {
      await _firebaseService.saveUberEatsMargins(
      ...
      children: [
              ...
              children: [
                ...
                Row(
                  children: [
                    SizedBox(
                      width: 100,
                      child: _MarginField(controller: _defaultMarginCtrl),
                    ),
        ...
        if (_allTags.isEmpty)
          const Padding(
            ...
            child: Center(child: Text('No tags found on Shopify')),
          )
        else
          Card(
            child: Column(
